...
theorem nextCoord_eq_of_localPairResidue
  ...
  -- congr_arg₂ on the structure constructor; eta makes LHS defeq to nextCoord.
  exact
    congr_arg₂
      (fun (rr qq : ℕ) => ({ r := rr, q := qq } : BlockCoord))
      hpair.2 hq1.symm
...
/-!
The selected affine/parity kernel now lives in
`Antigrav.Prove2Me.AffineCore`. The unrelated experiments below remain in this
mathematical authority file.
-/
